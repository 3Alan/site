...
summary: |-
  ...
  1. 创建 Algolia 账号，获取 Application ID、Search-Only API Key 和 Admin API Key。
  2. 在博客仓库中创建一个 Action，选择触发条件，运行 Algolia DocSearch 爬虫镜像。
  3. 在项目仓库中新建两个 Actions secrets，分别为 ALGOLIA_APP_ID 和 ALGOLIA_API_KEY。
