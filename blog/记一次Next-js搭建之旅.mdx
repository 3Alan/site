...
out_dated: true
summary: >-
  Next.js 是一个基于 React 的服务器端渲染框架，它可以生成静态 HTML 页面，也可以在服务器端渲染页面。Next.js 的优点包括更好的
  SEO、更快的首屏渲染速度以及更易于开发。


  Next.js 的项目结构与 React 开发不同，它使用路由映射和自带有路由，并且渲染方式可以是预渲染或客户端获取数据。


  Next.js 工程化可以配置 Eslint+Prettier、配置 alias、封装 axios、自定义 input hook、封装
  Dialog、移动端适配以及使用 Docker+coding 实现自动化部署。


  在使用 Next.js 时可能会遇到一些问题，例如环境变量配置和判断环境。
---
