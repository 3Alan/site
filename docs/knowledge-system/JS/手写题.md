...
summary: |-
  1. Promise类实现了ES6的Promise规范，包括`then`、`catch`、`finally`等方法。
  2. `new`操作符可以创建新对象，并调用构造函数。
  3. `instanceof`操作符用于判断一个对象是否属于某个类。
  4. `call`和`apply`方法可以改变函数的执行上下文。
  5. `bind`方法可以创建新函数，该函数的this值被绑定到指定值。
---
