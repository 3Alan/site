...
sidebar_position: 2
summary: >-
  前端工程化涉及到包管理工具和代码可维护性两个方面。


  包管理工具方面，npm1/npm2 采用嵌套树结构，存在重复安装问题。npm3/yarn 将依赖拍平后放在根目录下，但存在安全性和不确定性问题。pnpm
  速度快，支持 monorepo 和 hard link，更适合大型项目。


  代码可维护性方面，包括可分析性/可读性、可改变性/可拓展性和稳定性。可分析性/可读性可以通过各种 lint 工具和 source-map
  定位来提高。可改变性/可拓展性可以通过模块化和松耦合来实现。稳定性可以通过测试覆盖率来提高。
---
