...
summary: >-
  本文介绍了评论系统、邮件回复功能、部署 Valine-Admin、添加百度统计、使用 Hexo-Admin 管理、生成永久链接、给外链添加
  nofollow、搜索引擎 URL 自动上传、使用 Vercel 部署 Hexo 等功能。还列出了待办事项，包括优化
  SEO、微调样式、去除消耗加载速度的插件、使用 CDN 等。最后，作者提到在 Mac 上部署时遇到页面 404 的问题，并指出这是由于 Node
  版本造成的，使用对应的 Node 版本即可解决。
---
