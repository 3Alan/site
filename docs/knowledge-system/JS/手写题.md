...
summary: |-
  * Promise类模拟实现，包括then、race、all、allSettled等方法。
  * new函数模拟实现，通过原型链实现继承。
  * instance of运算符模拟实现，通过原型链查找。
  * call和apply函数模拟实现，通过改变this指向来调用函数。
  * bind函数模拟实现，返回一个新的函数，可以作为构造函数使用。
  * 柯里化函数模拟实现，将多参数函数转化为单参数函数。
  * compose函数模拟实现，将多个函数组合成一个函数。
  * 类型判断函数模拟实现，判断变量的类型。
---
