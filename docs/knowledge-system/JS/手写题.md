...
summary: |-
  1. Promise 实现类，包括 resolve、reject、then 方法等。
  2. new 实现类，模拟了 `new` 关键字的行为。
  3. instance of 实现类，判断一个对象是否为某个类的实例。
  4. call 和 apply 实现类，模拟了 `call` 和 `apply` 方法的行为。
  5. bind 实现类，模拟了 `bind` 方法的行为。
---
