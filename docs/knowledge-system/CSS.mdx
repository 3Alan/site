...
sidebar_position: 4
summary: >-
  CSS 是一种用于描述网页如何呈现的语言。它可以控制网页的布局、字体、颜色和动画。CSS 中有许多不同的单位，包括 `rem`、`em`、`vh` 和
  `vw`。`translate` 和绝对定位的区别在于，`translate` 会占据原来的位置，而绝对定位会脱离文档流。BFC
  是一个隔离的独立容器，容器中的子元素不会影响到外面的元素。创建 BFC 的方法有很多，包括根元素、`float` 不为 `none`、`position:
  absolute/fixed`、`display: inline-block/table-cell`、`overflow` 不为 `visible`
  和弹性盒子。垂直居中可以通过 flex 布局、绝对定位、table-cell 等方式实现。
---
