...
summary: >-
  * 前端面试题总结，涉及 HTML、JS、计算机网络、CSS 等知识点。

  * JS 部分包括原型及继承、深拷贝和浅拷贝、for in 和 for
  of、事件委托、实现滑动动画、eventLoop、this、闭包、Promise、防抖和节流等内容。

  * 前端框架相关部分包括 React 的生命周期、useRef、cloneElement、HOC 等知识点。
---
