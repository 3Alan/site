...
sidebar_position: 2
summary: >-
  使用 Github Actions 运行 Algolia DocSearch 实现 Docusaurus 搜索功能。首先需要创建 Algolia
  账号，然后在项目仓库中创建 Github Actions 和两个 Actions secrets。接着在项目根目录下新建 docsearch.json
  文档，并配置 docusaurus.config.js 文件。完成以上步骤即可实现搜索功能。
---
