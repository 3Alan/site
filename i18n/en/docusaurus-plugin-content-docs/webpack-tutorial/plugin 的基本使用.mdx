...
description: >-
  ...
  plugin, including common plugin (html-webpack-plugin, clean-webpack-plugin to
...
sidebar_position: 3
summary: >-
  This section introduces the usage of some common plugins of webpack.
  ...
  output's publicPath can help us put some files in cdn to speed up access after
  packaging.
...
In the first section of my summary [What is webpack](/posts/webpack-tutorial/what-is-webpack), it is mentioned that `index.html` needs to be manually moved to the `dist` directory after packaging. So can we leave this work to the program? 🤔
...
The official website introduces that it can automatically create HTML files for us, and can also use custom HTML templates.
...
module.exports = {
  mode: 'development', // The default is production
  entry: {
...
`npm run build` to package and see the result
...
The HtmlWebpackPlugin plugin automatically generates index.html for us in the `dist` directory, but... When the browser opens index.html, you can see nothing. What's the matter? Let's take a look at the generated index.html file and find the problem
...
There is no `#root` node in the generated file. What should I do? We also need to configure HtmlWebpackPlugin. The introduction of the official website mentioned earlier
...
We need to provide an HTML template by ourselves, so create an index.html in the src directory as the packaging template for HtmlWebpackPlugin
...
Pack again and take a look at the `index.html` file generated in the `dist` directory. It is the same as the html template we defined.
...
When you open the browser, you can see that Hello Webpack is also displayed correctly. 🎉
...
In order to verify whether the dist directory is cleared, we first create a file my.js in the dist directory by ourselves
