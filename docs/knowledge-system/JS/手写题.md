...
summary: >-
  Promise 是一个类，它允许你处理异步操作。你可以使用它来处理异步代码，并确保在异步操作完成时执行某些操作。Promise
  有三种状态：等待、完成和拒绝。你可以使用 then() 方法来处理 Promise 的完成和拒绝状态。
  ...
  call() 和 apply() 方法用于调用函数。它们都接受一个参数列表，并将这些参数传递给函数。call() 方法将第一个参数作为函数的 this
  值，而 apply() 方法将第一个参数作为函数的 this 值，并将第二个参数作为函数的参数列表。


  bind() 方法用于创建一个新的函数，该函数在调用时将使用指定的 this 值。它接受一个参数列表，并将这些参数传递给新函数。
---
