...
summary: >-
  Mitt 是一个非常简单的发布-订阅库，它只有不到 100 行代码。它使用一个 Map 来存储事件和它们的处理函数。你可以使用 `mitt.on()`
  来订阅事件，`mitt.off()` 来取消订阅事件，`mitt.emit()` 来触发事件。Mitt 还支持一次性事件，你可以使用
  `mitt.once()` 来订阅一次性事件。


  Mitt
  使用发布-订阅模式，它与观察者模式非常相似。在发布-订阅模式中，发布者将事件发布到事件总线上，订阅者可以订阅事件并接收事件通知。在观察者模式中，被观察者将事件通知给观察者，观察者可以处理这些事件。
---
