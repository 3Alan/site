...
draft: true
summary: >-
  * 小红书面试官问了自定义 hook、hook 和 class 的差异、算法题、ts 声明问题、宏任务微任务题目。

  * XTransfer 面试官问了 flex、垂直居中、用 css 画三角形、http2 多路复用原理、浏览器缓存、跨域问题、esm 和 cjs
  区别、rollup 和 webpack 区别、webpack 优化、性能优化、埋点、React diff 算法、React Redux connect
  原理、React Fiber 架构、hook 在 if 中使用、函数柯里化、Promise.all、banner 图加载模糊处理、闭包。

  * 黑湖面试官问了最难的问题、hook 和 class 区别、react 性能优化、长列表优化、webpack loader 和 plugin
  区别、webpack 优化、js 垃圾回收机制、promise resolve 和 reject 都没有触发、小程序偏底层、react diff
  算法、Fiber 解决了什么问题、埋点、错误收集、margin 重叠、编程题。

  * ebay 外包面试官问了 http 方法和状态码、restful api、cookie、登录权限验证、ssr、ssr 开发组件注意事项、ssr
  中服务端如何处理 css、svg、img、打包为什么用 rollup 不用 webpack、单元测试、class component 和 function
  component 区别、hook 能不能写在条件分支里、组件重新渲染的问题、memo 引用对象对比的问题、线上错误的追踪、koa 及中间件。
---
