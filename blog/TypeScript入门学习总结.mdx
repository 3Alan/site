...
authors: Alan
summary: >-
  TypeScript 是一种流行的 JavaScript 超集，它增加了可选的静态类型和基于类的面向对象编程。它可以编译成纯 JavaScript，未编译的
  TypeScript 代码无法在浏览器执行。


  TypeScript 的优点包括：


  - 能够进行动态类型检测，可以检测出一些潜在的 bug（例如拼写错误、参数缺失、undefined 等），提升代码健壮性
  ...
  TypeScript 中的类和 ES6 中的类很相似，但有一些不同之处，包括：


  - 变量修饰符：public（默认）、private（私有）、protected（受保护）

  - 静态属性：只能通过类来访问，不能通过实例访问

  - 构造函数：可以用来初始化类的属性

  - 抽象类：不能被实例化，要使用的话要声明其派生类，并且重写其中的抽象方法


  TypeScript 中的泛型是指在定义函数、接口或类的时候，不预先指定具体的类型，而在使用的时候再指定类型的一种特性。泛型可以用于函数、类和接口。


  TypeScript 在 React 中的应用包括：
