...
sidebar_position: 1
summary: >-
  利用 Gemini AI 和 Github 机器人，实现自动化生成文章摘要。首先，开发了一个 Github 机器人，将其部署到 Zeabur。然后，通过
  Github App 和 Webhook 将机器人与仓库关联起来。接着，新建一个 issue 并添加 `summarizer`
  标签，机器人就会开始总结博文。最后，在文章页面上安装三方库 `typed.js` 并修改组件，即可显示摘要。
---
