...
sidebar_position: 4
summary: |-
  webpack 的 devServer 可以起一个本地服务并且实现代码的热更新，省去每次更新代码后重启服务的操作。

  HMR（Hot Module Replacement）可以实现局部热更新，当我们更改了部分文件后，只需要更新这一部分内容。

  在 webpack.config.js 中配置 devServer 和 hot 选项即可开启热更新和 HMR。

  如果需要实现局部热更新，需要在代码中使用 module.hot.accept() 方法来监听文件变化。

  devServer 和 HMR 可以大大提高我们的开发效率。
---
