...
sidebar_position: 6
summary: >-
  1. 使用 ts-loader 打包 TS 文件，并配置 tsconfig.json 和 webpack.config.js。

  2. 使用 Eslint 统一代码风格，并结合 webpack 使用 eslint-loader 进行代码检查。

  3. 多页应用打包时，可以使用 HtmlWebpackPlugin 自动生成多个 html 文件。

  4. webpack 性能优化包括减少不必要的 Loader 和 Plugin 的使用，合理配置 Resolve，使用 DllPlugin
  优化第三方模块的打包速度，使用多进程打包，根据不同环境配置不同的 sourceMap，以及通过 status.json 文件分析打包过程。
---
