...
summary: >-
  最近利用业余时间写了一个借助 AI 来帮助你阅读理解文本的程序，支持阅读 pdf、markdown、html、txt 格式的文本。


  该程序的实现原理是：将 PDF 文本片段转换为向量，并使用余弦相似度算法匹配用户提出的问题和文本片段，从而实现对长文本的问答。


  这里用到了市面上功能比较完善的 `llama-index`，它提供了多种检索方式。


  在语义上对文本进行分割，使用 md 可以很好地做到这一点。


  Markdown 文件在生成索引时对最终展示的 html 进行里处理，将 html 元素节点和索引中的 chunk 块进行了关联，这样就可以实现高亮
  source 的功能。
---
