...
sidebar_label: 数组
summary: |-
  1. 数组去重的方法包括使用 `Set`、`filter` 和 `indexOf`。
  2. 数组填充可以使用 `fill` 方法或 `Array.from` 方法。
  3. 数组扁平化可以使用 `flat` 方法、递归或扩展运算符。
  4. 求数组最大值可以使用 `Math.max`、`sort` 或扩展运算符。
  5. 字符数组和对象数组都可以使用 `sort` 方法进行排序。
---
