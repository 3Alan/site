...
sidebar_position: 4
summary: >-
  webpack 可以使用 babel 来处理 ES6 语法，需要安装 `babel-loader`、`@babel/core` 和
  `@babel/preset-env`。配置 `webpack`，在 `.babelrc` 文件中配置 `presets`，使用
  `@babel/preset-react` 处理 React 文件，使用 `@babel-polyfill` 处理低版本浏览器无法处理的语法。
---
