...
summary: >-
  本文介绍了如何使用 Valine 评论系统、Valine-Admin 后台管理系统、百度统计、Hexo-Admin 管理系统、Hexo-Abbrlink
  生成永久链接、Hexo-Filter-Nofollow 给外链添加 nofollow
  属性、Hexo-Submit-Urls-To-Search-Engine 搜索引擎 URL 自动上传、Vercel 部署 Hexo
  等。还列出了后续需要完成的任务，如优化 SEO、样式微调、去除消耗加载速度的插件、使用 CDN 等。
---
