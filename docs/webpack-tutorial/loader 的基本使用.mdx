...
sidebar_position: 2
summary: >-
  本节主要介绍了一些常见的 Loader 的使用，包括 file-loader、url-loader 以及 css-loader 和
  style-loader。这些 Loader 可以帮助我们打包图片、样式文件、字体文件等。我们可以通过配置 Loader 的 options
  来控制打包后的效果，比如可以指定输出路径、限制文件大小等。通过使用
  Loader，我们可以方便地将各种类型的文件打包成一个文件，从而提高页面的加载速度和性能。
---
