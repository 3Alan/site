...
sidebar_position: 5
summary: >-
  Webpack4.0 的生产构建包括 Tree Shaking、开发环境和生产环境配置文件、代码分割、打包分析工具、代码优化、css 文件处理、配置
  output 解决缓存问题和 shimming。Tree Shaking 可以按需打包 js
  文件，只在生产环境下生效。为生产和开发环境分别创建不同的配置文件，方便管理扩展。利用代码分割实现懒加载和预加载。利用 status.json
  来分析打包过程。单独生成 css 文件，减少 main.js 体积。为 output 文件设置哈希值，解决缓存问题。
---
