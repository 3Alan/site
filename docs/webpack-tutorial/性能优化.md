...
summary: |-
  ...
  1. 减少不必要的 Loader 和 Plugin 的使用。
  2. 合理使用 resolve 的 extensions、mainFiles 和 alias。
  3. 使用 DllPlugin 优化项目中第三方模块的打包速度。
  4. 使用多进程打包插件提高打包速度。
  5. 根据不同环境配置不同的 sourceMap。
  6. 通过 status.json 文件分析打包过程，找到最影响性能的地方。
---
