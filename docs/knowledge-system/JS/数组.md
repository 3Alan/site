...
sidebar_label: 数组
summary: |-
  1. 数组去重有三种方法：循环遍历、filter 过滤、Set 对象。
  2. 数组填充有两种方法：new Array() 和 Array.from()。
  3. 数组扁平化有四种方法：Array.prototype.flat()、for of 递归、扩展运算符、骚操作。
  4. 求数组最大值有三种方法：Math.max()、Array.sort()、Math.max()。
  5. 字符数组排序和对象数组排序可以使用sort()方法。
---
