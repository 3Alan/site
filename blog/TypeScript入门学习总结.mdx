...
authors: Alan
summary: >-
  TypeScript 是一种强类型的 JavaScript 超集，它可以进行动态类型检测，可以检测出一些潜在的 bug，提升代码健壮性。TypeScript
  支持与 JavaScript 几乎相同的数据类型，包括基本类型、数组、元组、枚举、联合类型、类型断言、非空断言、接口、类、泛型等。TypeScript
  还支持 React 相关的内容，如 class 组件动态设置 state 等。
---
