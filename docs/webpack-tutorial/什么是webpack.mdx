...
sidebar_position: 1
summary: >-
  Webpack 是一个模块打包器，它可以将多个模块打包成一个或多个 bundle。Webpack 的配置文件是
  webpack.config.js，可以通过修改这个文件来配置 webpack 的打包行为。Webpack 可以使用 npm run xxx
  命令来打包项目，也可以使用自定义配置文件来打包项目。Webpack 打包好的 bundle 需要手动移动到 dist 目录下，并修改 index.html
  中的 js 路径。
---
