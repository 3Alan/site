...
sidebar_position: 4
summary: >-
  我们可以使用 webpack 的 devServer 和 HMR 来优化我们的开发效率。devServer 可以起一个本地服务并且实现代码的热更新，HMR
  可以实现局部热更新。配置 devServer 时，我们可以指定 contentBase、open 和 port 等选项。配置 HMR 时，我们需要在
  webpack.config.js 中设置 hot 选项为 true，并在需要热更新的模块中添加 module.hot.accept()
  方法。这样，当我们更改代码时，只有更改的部分会被重新渲染，从而提高开发效率。
---
