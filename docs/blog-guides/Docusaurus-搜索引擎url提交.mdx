...
sidebar_position: 3
summary: >-
  使用 Github Actions 自动提交 Docusaurus 网站的 URL 到搜索引擎（百度、必应、谷歌），以提高网站的收录率。使用前提是网站已生成
  sitemap。创建 Github Action 并配置相应的 secrets，即可实现自动提交。若配置了 Bing API Key，则提交 URL
  的数目应小于 10，以免超出每日配额。若要生成最近更新的 10 篇文章的 sitemap，目前尚无合适的解决方案，因为 Docusaurus 的
  sitemap 插件无法自定义生成规则。
---
