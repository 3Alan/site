...
summary: >-
  内存管理包括模块化、Promise、async/await、面试题、基础点、不同类型的比较、对象转原始类型、原始类型方法、结构赋值、Symbol.iterator、WeakMap
  和 Map 的不同、Map 和 Set、Set 和 WeakSet、垃圾回收、ES6、文件上传、ESM 和 CJS、滚动
  Tab、错误捕获、cookie、interface 和 type 的不同、React、vdom、diff dom、合成事件、class 和
  hooks、useEffect 和 useLayoutEffect、React Router。
---
