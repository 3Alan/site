...
summary: >-
  - 内存管理：严格模式、模块化、Promise、Async/Await。

  - 面试题：交换值、可选链、不同类型的比较、对象转原始类型、结构赋值、Symbol.iterator、WeakMap 和 Map 的不同、Set 和
  WeakSet、垃圾回收、ES6、文件上传、ESM 和 CJS、滚动 Tab、错误捕获、cookie、interface 和 type 的不同。

  - React：VDOM、Diff DOM、合成事件、Class 和 Hooks、useEffect 和 useLayoutEffect、React
  Router。
---
