...
authors: Alan
summary: >-
  - React 采用单向数据流，组件名首字母大写，可以使用 `<React.Fragment></ React.Fragment>` 或 `<></>`
  包裹标签。

  - 组件通信有父组件向子组件传递数据和子组件向父组件传递数据两种方式。

  - 可以使用 props
  参数校验及默认值，也可以使用异步组件加载插件[react-loadable](https://github.com/jamiebuilds/react-loadable)。

  - 虚拟 DOM 是一种使用 JS 对象来描述真实 DOM 的方式，它可以提高性能。

  - React 有自己的生命周期，包括 mounting、updation 和 unmounting。

  - Redux 是一个状态管理工具，它可以帮助管理组件之间的状态。

  - React-router 是一个路由库，它可以帮助管理页面的路由。

  - React 和 Vue 的对比：React 学习成本较高，但比较灵活；Vue 提供了很多封装好的 API，学习起来对小白比较友好。
---
