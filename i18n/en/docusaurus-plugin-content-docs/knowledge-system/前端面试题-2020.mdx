...
summary: >-
  ...
  The article includes JS prototypes and inheritance, deep copy and shallow
  copy, for in and for of, event delegation, implementation of sliding
  animation, eventLoop, this, closures, Promise, anti-shake and throttling, JS
  questions, etc.

  The article also involves related knowledge of the React framework, such as
...
You can view Chapter 6 of the Red Book (which is very "dry") for more inheritance methods and their advantages and disadvantages.
...
<Tabs>
  <TabItem value="Basic Type" label="Basic Type">
...
```
  ...
  <TabItem value="Reference Type" label="Reference Type">
...
From the examples above, we know that cloneObj and obj point to the same address. Any modification by either party will affect the other. So how do we create an independent cloneObj? This requires the use of deep copy and shallow copy.

The difference between deep copy and shallow copy:

Distinguish according to the level of copying. Shallow copy only copies one level, while deep copy copies multiple levels.
...
- for in traverses the enumerable properties in the array, including the prototype. Can traverse objects, traversing key values
- for of only traverses the elements of the array or iterable object, excluding the prototype. Traverses value values.
...
Event delegation takes advantage of event bubbling, using only one event handler to manage all events of a certain type.

For example, the click event will bubble up to the document level. For example, in the following example, we do not need to add an onclick event to all li elements, we just need to use the event bubbling feature to achieve event delegation.
