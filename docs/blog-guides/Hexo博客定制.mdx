...
summary: >-
  本文介绍了评论系统和一些自定义配置。包括如何注册 LeanCloud 账号、部署 Valine-Admin、添加百度统计、使用 hexo-admin
  管理、使用 hexo-abbrlink 生成永久链、给外链添加 nofollow、搜索引擎 URL 自动上传、使用 Vercel 部署 Hexo
  等。最后还列出了一些待办事项，如优化 SEO、微调样式、去除消耗加载速度的插件、使用 CDN 等。
---
