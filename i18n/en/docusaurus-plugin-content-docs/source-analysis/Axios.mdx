...
description: >-
  ...
  library is implemented, including the implementation of functions such as
  adapter, interceptor, request interruption, and timeout interruption.
summary: >-
  ...
  - Request interruptions and timeout interruptions can be achieved through
...
- Adapter pattern
- [Simple use of Fetch](https://developer.mozilla.org/zh-CN/docs/Web/API/Fetch_API/Using_Fetch)
...
The interceptor is a function that we use more often.
...
- Response interceptor: handle code codes (301 redirect, 404 redirect to notFound page), collect error logs for failed requests, mock data, uniformly process data return formats, record request time, etc...
...
axios.interceptors.response.use(
  function responseInterceptorFulfilled(response) {
    ...
    // Interface request time consumption
...
In the example above, we add `header.token` and `metadata` to the request parameters, `extraData` and `duration` (record request time consumption) to the request return body, and handle the 404 status code.

### Specific Implementation

`InterceptorManager` maintains an array internally, adding (`use`) and deleting (`eject`) operations on the interceptor
...
export default class InterceptorManager {
  ...
  use(fulfilled, rejected) {
    ...
    // Return the index of the current handler in the array, used for eject
...
request(config) {
  ...
  // Convert config to a promise object to facilitate the formation of a Promise chain later
