...
out_dated: true
summary: >-
  Next.js 是基于 React 的 SSR（服务端渲染框架），它可以提供更好的 SEO 和更快的首屏渲染速度。Next.js 的路由方式与 React
  Router 类似，可以使用编程式跳转和组件式跳转。在 Next.js 中，可以通过 `getStaticProps` 和
  `getServerSideProps` 来获取数据，并使用 `SWR` 来在客户端获取数据。Next.js
  还提供了很多开箱即用的功能，如自动代码分割、路由预取、国际化支持等。在项目中，我使用了 Next.js 来构建一个移动端 H5
  活动页面，并遇到了很多问题，如环境变量配置、判断环境、移动端适配等。最终，我通过查阅官方文档、搜索相关资料和向同事请教，解决了这些问题，并成功地将 H5
  活动页面迁移到了 Next.js 下。
---
