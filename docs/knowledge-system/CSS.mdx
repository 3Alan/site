...
sidebar_position: 4
summary: >-
  1. CSS 中单位有 `rem`、`em`、`vh/vw` 等，它们分别相对于根元素的字体大小、元素的字体大小、视窗的高度或宽度。

  2. `translate` 和绝对定位的区别在于，`translate` 会占据原来的位置，而绝对定位会脱离文档流。

  3. BFC 是一个隔离的独立容器，容器中的子元素不会影响到外面的元素，可以通过根元素、`float` 不为 `none`、`position:
  absolute/fixed` 等方式创建。

  4. 清理浮动的方法有伪类元素、创建 BFC、空盒子等。

  5. CSS 选择器优先级为 `!important > style > id(100) > class(10) > 标签(1) > * > 继承`。
---
