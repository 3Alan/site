...
summary: >-
  双指针算法是一种常见的算法思想，它使用两个指针来遍历数据结构。相向型双指针从两端向中间移动，同向型双指针从一端向另一端移动，背向型双指针从中间向两端移动。双指针算法可以用于解决各种问题，如验证回文串、删除链表的倒数第
  N 个结点、环形链表、反转链表等。滑动窗口是一种特殊的双指针算法，它用于在数据流中查找满足某种条件的子序列。
---
