...
sidebar_label: 数组
summary: |-
  - 数组去重：使用Set、filter或indexOf方法。
  - 数组填充：使用fill方法或Array.from方法。
  - 数组扁平化：使用flat方法、递归或扩展运算符。
  - 求数组最大值：使用Math.max方法、sort方法或扩展运算符。
  - 字符数组排序：使用sort方法比较数组元素的字符编码。
  - 对象数组排序：使用sort方法比较数组元素的某个属性。
  - map相关：map方法可以将数组中的每个元素都映射成一个新值，它接受三个参数（item, index, array）。
---
