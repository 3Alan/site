...
sidebar_position: 1
summary: |-
  1. 在 Docusaurus 中添加评论功能需要安装 `@giscus/react` 和 `mitt` 两个包。
  2. 创建一个评论组件，并在其中使用 `Giscus` 组件来实现评论功能。
  3. 通过 `Docusaurus Swizzling` 将评论组件添加到文档和博客页面。
  4. 在 `docusaurus.config.js` 中配置 `giscus` 选项，包括仓库、类别和主题等信息。
  5. 完成以上步骤后，你就可以在每篇博客和每篇文档的底部看到评论功能了。
---
