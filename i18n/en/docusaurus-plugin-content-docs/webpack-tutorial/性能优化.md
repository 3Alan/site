...
summary: >-
  ...
  3. Eslint is used to unify the code style. You can use eslint-loader to check
  the code during packaging.
  ...
  alias, using DllPlugin, multi-process packaging, sourceMap and other tools.
ai_translation: true
---
...
`tsconfig.json` is the configuration file for TS. For more configurations, [see](https://www.typescriptlang.org/docs/handbook/tsconfig-json.html)
...
Single-page applications need to be configured in the development environment `devServer.historyApiFallback： true`

### Eslint

Eslint is used to unify the code style. When collaborating in a team, it can ensure that the code style written by everyone is unified, which is conducive to code maintenance.
...
The picture above shows that eslint prompts that jsx syntax cannot be used in js files, this....😰. What should I do if I want to use it? You can configure rules in the generated `.eslintrc.js` file to ignore this standard.
...
Since it is difficult to ensure that everyone's development environment is the same in team collaboration, we can combine eslint and webpack, and use a loader to check the code for eslint every time it is packaged, but this may affect the packaging speed. When using it, you need to make a choice between speed and code quality. Here is an introduction to the eslint-loader used
...
**Now there are only two pages, which is okay. However, when there are more pages, we need to configure multiple HtmlWebpackPlugin. So how can we make it configure automatically? Here is a function to handle it**
