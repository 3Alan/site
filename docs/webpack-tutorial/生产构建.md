...
sidebar_position: 5
summary: |-
  1. Tree Shaking 可以去除没有使用到的代码，优化打包产物体积大小。
  2. 开发环境和生产环境分别创建不同配置文件，方便管理扩展。
  3. 代码分割有利于性能优化，同步代码分割可以将公共代码和业务代码分开打包，异步代码分割可以实现懒加载。
  4. 利用 status.json 文件可以分析打包过程。
  5. MiniCssExtractPlugin 可以将 css 打包到一个 css 文件中，通过 link 链接到 html 中。
  6. 为 output 文件设置 hash 值，解决缓存问题。
---
