...
summary: >-
  Mitt 是一个轻量级的事件派发器，它使用 Map 来存储事件和处理函数。Mitt 提供了 `on()`、`off()`、`emit()` 和
  `once()` 等方法来操作事件。Mitt 的 `emit()` 方法会先触发指定事件的处理函数，然后触发 `*` 事件的处理函数。Mitt 的
  `once()` 方法可以注册一个只触发一次的处理函数。Mitt 的 `clear()` 方法可以清除所有事件和处理函数。Mitt 的源码非常简易，只有不到
  100 行。
---
