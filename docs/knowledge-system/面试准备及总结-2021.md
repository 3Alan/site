...
summary: >-
  - 前端面试题总结，涵盖内存管理、模块化、Promise、async/await、JS 基础、结构赋值、Symbol.iterator、WeakMap 和
  Map、Set 和 WeakSet、垃圾回收、ES6、文件上传、ESM 和 CJS、滚动 Tab、错误捕获、cookie、interface 和 type
  的不同、React、React Router 等知识点。

  - 不同类型的比较中，当对不同类型的值进行比较时，JavaScript 会首先将其转化为数字再判定大小。

  - WeakMap 和 Map 的不同之处在于，WeakMap 的键必须是对象，不能是原始值，并且 WeakMap 不支持迭代以及
  keys()、values() 和 entries() 方法。

  - Set 和 WeakSet 的不同之处在于，WeakSet 仅存储对象。

  - 垃圾回收有两种类型：标记清除和引用计数。

  - ES6 中 var 和 const/let 的不同之处在于，var 是函数作用域，const/let 是块级作用域，并且 const/let
  不能重复声明。
---
