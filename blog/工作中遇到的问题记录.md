...
summary: |-
  **记录了作者在工作中遇到的问题和解决方法：**
  ...
  - **Vue**：
    - 滚动穿透问题：使用 `@touchmove.stop`。
  - **React**：
    - 使用 `switch` 时，需要考虑不同路由下组件的渲染顺序。
    - `localstorage` 和 `sessionstorage` 的生命周期。
    - 在 `useEffect` 中使用一步函数。
    - 使用 `defaultProps`。
  - **CSS**：
    - `pre` 不能换行的问题：使用 `white-space: pre-wrap; word-break: break-all;`。
    - 移动端 `hover` 问题：参考 [stackoverflow](https://stackoverflow.com/questions/23885255/how-to-remove-ignore-hover-css-style-on-touch-devices)。
    - `text-decoration` 的传播性问题：参考 [stackoverflow](https://stackoverflow.com/questions/40829232/why-does-the-text-decoration-none-not-work-inside-p)。
  - **待学习**：
    - `git` 的 `fast-forword`。
    - `cherry-pick`。
    - `github fork`。
  - **mac 使用以及 vscode 技巧**：
    - 快捷键、终端代理、vscode 快捷键、软件推荐。
  - **mpvue**：
    - 生命周期、`mpvue-echarts`、其他。
  - **Docker**：
    - 命令、`Dockerfile` 书写示例。
  - **ReactNative**：
    - H5 调起 APP。
  ...
  - **配置 alias**：
    - 修改 `jsconfig` 文件。
  - **环境处理**：
    - 使用 `cross-env` 处理不同平台问题。
    - 创建文件用于处理不同环境下的 API。
  - **移动端适配**：
    - 使用 `postcss-px-to-viewport`。
  - **移动端保存图片到本地**：
    - 参考 [cnblogs](https://www.cnblogs.com/zhangkaiqiang/p/8183926.html)。
  - **nvm 管理多 node 版本**：
    - 常用命令。
  - **多仓库的问题**：
    - 使用 `npm link`。
  - **typescript 相关**：
    - 动态设置 `state` 值时 `typescript` 类型推导问题。
    - `export type` 的相关问题。
  - **`git` 回退上次的 `commit`**：
    - `git reset --soft HEAD^`。
  - **`git` 文件大小写被忽略的问题**：
  ...
  - **html email**：
    - 由于不同邮件客户端对 `css` 的兼容性不同，导致 `css` 很多属性不能使用。
    - 有些客户端不支持 `<head><style></style></head>`，所以样式使用行内样式。
