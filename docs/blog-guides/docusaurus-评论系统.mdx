...
sidebar_position: 1
summary: |-
  1. 按照 Giscus 官网的步骤配置安装并获取配置参数。
  2. 安装所需包 `yarn add @giscus/react mitt`。
  3. 封装评论组件，包括配置生命周期函数和创建组件。
  4. Swizzling Docusaurus 内部组件，分别对文档页面和博客页面对应组件进行修改。
  5. 配置 Giscus，包括设置仓库、分类等信息。
  6. 完成以上步骤后，即可在每篇博客和每篇文档的底部看到评论功能。
---
