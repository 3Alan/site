...
draft: true
summary: >-
  以太坊是一个去中心化的区块链平台，它使用智能合约来实现各种应用。智能合约是一种自动执行的计算机程序，它存储在区块链上，并根据预先定义的规则运行。以太坊的交易费由
  gas 计算，gas 是用来衡量进行一个交易所要消耗的计算资源，交易费如何计算取决于以太坊升级前后的规则。加密钱包用来管理账户，连接应用。以太坊开发使用
  Solidity 语言，运行在 Ethereum Virtual Machine(EVM) 上。
---
