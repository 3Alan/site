...
summary: >-
  Hexo 博客定制指南，包括评论功能、后台管理、百度统计、搜索引擎收录及 Vercel 部署。评论功能使用了 LeanCloud 国际版和 Valine
  国际版，后台管理使用 hexo-admin，百度统计通过复制统计代码并粘贴到网站中实现，搜索引擎收录使用
  hexo-submit-urls-to-search-engine 插件，Vercel 部署通过配置 package.json 和 vercel 来实现。
---
