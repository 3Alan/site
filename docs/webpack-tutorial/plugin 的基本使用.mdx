...
sidebar_position: 3
summary: >-
  本节介绍了 webpack 常用插件的使用，包括
  html-webpack-plugin、clean-webpack-plugin、多入口打包及配置、sourceMap
  配置。html-webpack-plugin 可以自动生成 HTML 文件并引入打包后的 JavaScript
  文件。clean-webpack-plugin 可以清除 dist 目录中的文件。多入口打包可以通过设置 entry 来完成。sourceMap
  可以帮助我们定位代码错误。
---
