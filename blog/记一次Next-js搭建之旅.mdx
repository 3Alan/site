...
out_dated: true
summary: >-
  Next.js 是一款基于 React 的 SSR 框架，它可以提供更好的 SEO 和更快的首屏渲染速度。Next.js 的路由方式是通过在 `pages`
  目录下创建 `.js`、`.jsx`、`.ts` 或 `.tsx` 文件来实现的，每个文件对应一个路由。Next.js
  还提供了很多开箱即用的功能，比如自动代码分割、服务端渲染、静态生成等。
  ...
  总的来说，Next.js 是一款非常优秀的 SSR 框架，它可以帮助开发者构建出高性能、可扩展的 Web 应用。
---
