...
summary: >-
  本文参考 Axios 源码，实现了一个简易的请求库，包含基本请求、适配器、拦截器、请求中断和超时中断等功能。


  适配器使 Axios 在不同平台上使用不同的 API 发送请求，本文使用 fetch 代替了 Axios Browser 中的 ajax。


  拦截器允许在请求发送前或响应返回后修改请求或响应数据。


  请求中断和超时中断允许在请求过程中取消请求或设置超时时间。
