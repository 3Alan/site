...
summary: >-
  Hexo 博客定制指南，包括评论功能、后台管理、百度统计、搜索引擎收录及 Vercel 部署。


  评论功能使用 LeanCloud 国际版，并部署 Valine-Admin 进行管理。添加百度统计，使用 Hexo-Admin 管理博客。


  使用 Hexo-Abbrlink 生成永久链接，并使用 Hexo-Filter-Nofollow 给外链添加 Nofollow 属性。使用
  Hexo-Submit-Urls-To-Search-Engine 插件自动提交 URL 到搜索引擎。


  使用 Vercel 部署 Hexo，配置 package.json 和 Vercel，即可实现一键部署。


  TODO：优化 SEO，微调样式，去除消耗加载速度的插件，使用 CDN。
---
