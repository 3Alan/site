...
out_dated: true
summary: >-
  Next.js 是一个基于 React 的 SSR 框架，它可以提供更好的 SEO 和更快的首屏渲染速度。Next.js 的路由方式是通过在 `pages`
  目录下创建 `.js`、`.jsx`、`.ts` 或 `.tsx` 文件来实现的，每个文件都对应一个路由。Next.js
  还提供了很多开箱即用的功能，比如自动代码分割、服务端渲染、静态生成等。在本文中，作者分享了他在使用 Next.js 时遇到的问题和解决办法，包括如何配置
  Eslint+Prettier、如何配置 alias、如何封装 axios、如何自定义 input hook、如何封装
