...
summary: >-
  ...
  - Vue 中的滚动穿透问题，可以使用 `@touchmove.stop` 来解决。

  - React 中的 `switch` 组件，可以使用 `https://segmentfault.com/a/1190000022444683` 来解决。

  - React 中的 `useEffect` 函数，可以使用一步函数来解决。

  - React 中使用 `defaultProps`，可以使用
  `https://stackoverflow.com/questions/37282159/default-property-value-in-react-component-using-typescript`
  来解决。

  - CSS 中的 `pre` 不能换行的问题，可以使用 `white-space: pre-wrap; word-break: break-all;`
  来解决。
---
