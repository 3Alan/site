...
sidebar_position: 5
summary: |-
  Tree Shaking 可以去除代码中未使用的部分，优化打包产物体积。
  开发环境和生产环境可以分别设置不同的配置文件。
  代码分割可以提高性能，减少加载不必要的代码。
  可以使用 Bundle Analyzer 来分析打包结果。
  MiniCssExtractPlugin 可以将 CSS 单独打包成一个文件。
  设置 output 的 filename，添加`[contenthash]`占位符，可以解决缓存问题。
  Shimming 可以解决第三方库中依赖的模块缺失的问题。
---
