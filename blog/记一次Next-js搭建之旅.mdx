...
out_dated: true
summary: >-
  Next.js 是基于 React 的 SSR（服务端渲染）框架，它可以提供更好的 SEO 和更快的首屏渲染速度。Next.js 的路由映射方式与
  React 不同，它使用 page（页面）的概念，每个 page（页面）都是一个从 .js、jsx、.ts 或 .tsx 文件导出（export）的
  React 组件，这些文件存放在 pages 目录下。Next.js 自带路由：next/router 和 next/link，使用与
  react-router 类似。Next.js 有两种渲染方式：预渲染和客户端获取数据。预渲染包括静态生成（Static
  Generation）和服务器端渲染（Server-side Rendering）。静态生成是 HTML 重用，build
  生成，服务器端渲染是每次请求生成的 HTML 不同，用户请求时生成。项目结构及工程化方面，Next.js 支持配置 Eslint+Prettier、配置
  alias、封装 axios 在每次请求时显示 spin 组件、自定义 input hook、封装 Dialog、移动端适配、使用
  Docker+coding 实现自动化部署。Next.js 踩坑方面，需要注意环境变量配置和判断环境。
---
