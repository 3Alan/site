...
summary: >-
  This article introduces the comment system, email reply function, deployment
  of Valine-Admin, addition of Baidu statistics, management using Hexo-Admin,
  generation of permanent links, addition of nofollow to external links,
  automatic upload of search engine URLs, deployment of Hexo using Vercel, etc.
  It also lists the to-do items, including optimizing SEO, fine-tuning styles,
  removing plugins that consume loading speed, using CDN, etc. Finally, the
  author mentions the problem of encountering page 404 when deploying on Mac,
  and points out that this is caused by the Node version, which can be solved by
  using the corresponding Node version.
ai_translation: true
---
...
### Register a LeanCloud International Edition account (domestic accounts seem to require domain name binding and备案)

Registration address 👉https://console.leancloud.app/login.html#/signup

Then create an application, name it whatever you want

Find the application keys in the settings, copy the appid and appkey to the corresponding configuration of hexo. Since this article focuses on the email reply function, this part will not be elaborated on 😝. Let's get to the point.
...
Click the link to access the background management system (you need to log in with a username and password here, you need to create it yourself in \_User in the storage)

![Create Storage](https://raw.githubusercontent.com/3Alan/images/master/img/image-20200516164422299.png)

Here you only need to fill in username, password, email, etc. After adding, you can successfully log in by entering your email and password in the background management system.
...
Check in the console network if your website has a request starting with **hm**, if so, it means it is successful, and you can see the data in Baidu Statistics after about 20 minutes.
...
If you want to use deploy in hexo-admin, just configure it in \_config.yml
...
## hexo-abbrlink Generates Permanent Links
...
Use [`hexo-filter-nofollow`](https://github.com/hexojs/hexo-filter-nofollow) to automatically add `nofollow ` attribute to external links to prevent external links from reducing the SEO weight of this site

## Search Engine URL Automatic Upload
...
When I deployed on mac, I found that the page was 404. After
