...
out_dated: true
summary: >-
  Next.js是一个基于React的SSR（服务端渲染框架），它可以提供更好的SEO和更快的首屏渲染速度。Next.js与React开发的不同之处在于它使用路由映射，并提供了`next/router`和`next/link`两个组件来进行路由管理。Next.js还提供了多种渲染方式，包括静态生成和服务器端渲染，以及相应的API。在项目结构和工程化方面，Next.js提供了对Eslint+Prettier的配置、别名的配置、axios的封装、自定义input
  hook、Dialog组件的封装、移动端适配等功能。在部署方面，Next.js可以与Docker+coding结合实现自动化部署。此外，Next.js还提供了对redux的支持。在使用Next.js的过程中，需要注意环境变量的配置和判断环境的方式。
---
