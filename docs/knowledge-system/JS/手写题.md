...
summary: |-
  Promise 类模拟实现，包括 then、race、all、allSettled 等方法。
  new 函数模拟实现，通过原型链实现继承。
  instance of 运算符模拟实现，通过原型链查找。
  call 和 apply 函数模拟实现，通过改变 this 指向来调用函数。
  bind 函数模拟实现，返回一个新的函数，可以作为构造函数使用。
---
