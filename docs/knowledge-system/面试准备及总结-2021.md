...
summary: >-
  - 内存管理：JavaScript 中的变量作用域、模块化、Promise、async/await 等概念。

  - 面试题：解决两个变量交换值的问题，以及不同类型比较的规则。

  - 对象转原始类型：如何将对象转换为原始类型，以及原始类型的方法。

  - 结构赋值：如何使用结构赋值来解构对象和数组。

  - Symbol.iterator：如何让对象变得可迭代，以及迭代器的使用。

  - WeakMap 和 Map 的不同：WeakMap 和 Map 的区别，以及 WeakMap 的应用场景。

  - Map 和 Set：Map 和 Set 的区别，以及 Set 的应用场景。

  - 垃圾回收：JavaScript 中的垃圾回收机制，以及如何避免内存泄漏。

  - ES6：ES6 中 var、const 和 let 的区别，以及文件上传的相关知识。

  - ESM 和 CJS：ESM 和 CJS 的区别，以及滚动 Tab 的实现。

  - 错误捕获：如何捕获运行时错误、语法错误和资源加载失败。

  - cookie：cookie 的设置和使用，以及 cookie 的安全属性。

  - interface 和 type 的不同：interface 和 type 的区别，以及 React 中 vdom、diff
  dom、合成事件、class 和 hooks 的相关知识。

  - React Router：React Router 的原理和使用，以及解决 404 问题的方法。
---
