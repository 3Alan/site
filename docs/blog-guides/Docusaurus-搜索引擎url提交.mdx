...
sidebar_position: 3
summary: |-
  通过 Github Actions 自动提交 Docusaurus url 到搜索引擎（百度、必应、谷歌），只要你的网站有 sitemap 就可以使用。

  使用前提是你的网站已经生成了 sitemap。

  创建 Github Action，配置 Actions secrets，即可自动提交 url 到搜索引擎。

  需要注意的是，如果配置了 BING_TOKEN，那么 COUNT 数值应小于 10，由于 BING 每日配额数只有 10 条。
