...
summary: |-
  - Promise 是一个 JavaScript 对象，用于处理异步操作。
  - new 运算符用于创建一个新对象。
  - instance of 运算符用于检查一个对象是否是另一个对象的实例。
  - call 和 apply 方法用于调用一个函数，并指定该函数的 this 值。
  - bind 方法用于创建一个新的函数，该函数在被调用时，其 this 值被指定为 bind 方法的第一个参数。
  - 柯里化是一种将一个函数转换为另一个函数的技术，该函数接受较少的参数，并返回一个新的函数，该函数接受剩余的参数。
  - compose 函数将多个函数组合成一个函数。
  - 类型判断函数用于判断一个变量的类型。
---
