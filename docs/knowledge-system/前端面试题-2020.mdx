...
summary: |-
  1. 原型和继承：组合继承可以实现多级继承，通过调用父类的构造函数来继承父类的属性和方法。
  2. 深拷贝和浅拷贝：深拷贝会对对象进行递归拷贝，而浅拷贝只拷贝一层。
  3. for in 和 for of：for in 遍历数组中的可枚举属性，包括原型，而 for of 只遍历数组的元素或者可以迭代的对象，不包括原型。
  4. 事件委托：利用事件冒泡，只指定了一个事件处理程序，就可以管理某一类型的所有事件。
  5. 实现滑动动画：利用 setInterval 和 requestAnimationFrame 实现平滑的动画效果。
---
