...
sidebar_position: 4
summary: >-
  Babel 是一个 JavaScript 编译器，可以将 ES6 代码转换为 ES5 代码，以便在旧浏览器中运行。


  要使用 Babel，需要安装 `babel-loader` 和 `@babel/core` 两个包，然后在 Webpack 配置文件中添加一个规则，将所有
  JavaScript 文件都交给 Babel 处理。


  Babel 可以处理 ES6 语法，也可以处理 React 的 JSX 语法。


  为了兼容低版本浏览器，可以使用 `@babel/polyfill` 来模拟 ES5 环境，或者使用 `@babel/preset-env` 的
  `useBuiltIns` 选项按需引入必要的 polyfill。
---
