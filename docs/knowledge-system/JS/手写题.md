...
summary: |-
  Promise 是一个类，它允许你异步地处理事件。
  它有三种状态：等待、成功和失败。
  你可以使用 then 方法来添加回调函数，当 Promise 改变状态时，这些回调函数就会被调用。
  Promise 还提供了静态方法 all、allSettled 和 race，分别用于处理多个 Promise 的情况。

  new 关键字用于创建一个新对象。
  它会调用构造函数，并将新对象作为构造函数的 this 对象。
  instanceof 运算符用于检查一个对象是否属于某个类。
  它会检查对象的原型链，看看它是否能找到该类的原型。

  call 和 apply 方法用于调用一个函数，并指定函数的 this 对象。
  bind 方法用于创建一个新的函数，该函数的 this 对象被绑定到指定的对象。

  柯里化是一种将函数转换为另一个函数的技术，该函数接受较少的参数，并返回一个新的函数，该函数接受剩余的参数。
  组合函数是一种将多个函数组合成一个函数的技术，该函数依次调用这些函数，并将前一个函数的返回值作为下一个函数的参数。

  类型判断函数用于判断一个变量的类型。
  它通常使用 Object.prototype.toString.call 方法来获取变量的类型字符串，然后根据该字符串来判断变量的类型。
---
