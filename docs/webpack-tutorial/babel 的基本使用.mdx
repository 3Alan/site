...
sidebar_position: 4
summary: |-
  使用 Babel 可以将 ES6 代码转换为 ES5 代码，从而兼容低版本浏览器。
  Babel 的核心内容是 @babel/core，而 @babel/preset-env 可以将 ES6 代码转换为 ES5 代码。
  @babel-polyfill 可以模拟 ES5 环境，但会增加打包后的文件大小。
  可以配置 useBuiltIns:usage 来按需引入 @babel-polyfill，从而减小打包后的文件大小。
  Babel 的配置可以单独放在 .babelrc 文件中。
---
