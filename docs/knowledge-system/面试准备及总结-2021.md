...
summary: |-
  * 内存管理涉及模块化、Promise、async/await 和 JavaScript 基础。
  * 结构赋值、Symbol.iterator、WeakMap 和 Map、Set 和 WeakSet 都是 ES6 中的新增特性。
  * 垃圾回收有两种类型：标记清除和引用计数。
  * ES6 中 var 和 const/let 的不同之处在于作用域和声明方式。
  * React 和 React Router 是前端开发中常用的框架和库。
---
