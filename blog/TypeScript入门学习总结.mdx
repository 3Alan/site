...
authors: Alan
summary: >-
  TypeScript 是一种 JavaScript 的超集，在 JavaScript 的基础上增加了可选的静态类型和基于类的面向对象编程。它可以编译成纯
  JavaScript，未编译的 ts 代码无法在浏览器执行。TypeScript
  常用知识点包括：基础类型、数组、元组、枚举、any、void、null、undefined、never、结构赋值的写法、联合类型、类型断言、非空断言、接口、类、泛型、React
  相关。
---
