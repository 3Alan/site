...
sidebar_position: 1
summary: >-
  Webpack 是一种模块打包器，它可以将多个模块打包成一个或多个文件。Webpack 的配置文件通常是
  `webpack.config.js`，它可以指定打包的入口文件、输出文件、模式等。Webpack 有两种模式：`development` 和
  `production`，`development` 模式下的代码不是一整行的有利于调试，而 `production`
  模式下的代码是一整行的，体积更小。Webpack 可以通过 `npm run build` 命令来打包项目。
---
