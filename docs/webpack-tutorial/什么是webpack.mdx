...
sidebar_position: 1
summary: >-
  Webpack 是一个模块打包器，它可以将多个模块打包成一个或多个 bundle。Webpack 的配置文件通常是
  webpack.config.js，它可以配置打包的入口文件、输出文件、模式等。Webpack 有两种模式：development 和
  production，development 模式下代码不是一整行的，有利于调试，production 模式下代码是一整行的，体积更小。Webpack
  可以通过 `npm run build` 命令来打包项目。
---
