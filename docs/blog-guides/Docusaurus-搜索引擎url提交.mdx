...
summary: >-
  ...
  可以将网站的 `sitemap` 提交给搜索引擎。

  使用前提是网站已经生成了 `sitemap`。

  在项目仓库中创建 `Github Action` 和 `Actions secrets`，然后就可以使用了。

  需要注意的是，如果配置了 `BING_TOKEN`，那么 `COUNT` 数值应小于 10，因为 `BING` 每日配额数只有 10 条。

  目前正在研究如何生成最近更新的 10 篇文章 `sitemap`。
---
