...
sidebar_position: 2
summary: >-
  由于 Algolia DocSearch 申请困难，我创建了一个自己的 DocSearch 爬虫。首先，我创建了一个 Algolia 账号并获取了三个
  key 值。然后，我在博客仓库中创建了一个 GitHub Actions，并根据需要选择触发条件。接下来，我在项目仓库中创建了两个 Actions
  secrets。最后，我创建了一个 docsearch.json 文件并配置了
  docusaurus.config.js。完成这些步骤后，我就可以实现搜索功能了。
---
