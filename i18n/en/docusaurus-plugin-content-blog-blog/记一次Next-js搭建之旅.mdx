---
slug: next-tutorial
title: Notes on the Next.js Setup Journey
tags:
  - React
  - Next.js
categories:
...
keywords:
  ...
  - Redux
description: >-
  Next.js setup tutorial, problems encountered during setup, front-end
  engineering setup
authors: Alan
out_dated: true
summary: >-
  ...
  method is similar to React Router, and programmatic and component-based
  navigation can be used. In Next.js, you can use `getStaticProps` and
  `getServerSideProps` to fetch data, and use `SWR` to fetch data on the client.
  Next.js also provides many out-of-the-box features, such as automatic code
  splitting, route prefetching, internationalization support, etc. In the
  project, I used Next.js to build a mobile H5 event page and encountered many
  problems, such as environment variable configuration, environment judgment,
  mobile adaptation, etc. In the end, I solved these problems by consulting
  official documentation, searching for relevant information, and asking
  colleagues for advice, and successfully migrated the H5 event page to Next.js.
ai_translation: true
---

During this time, work required that the previous vue H5 event page be transferred to Next.js. After a period of research, the following content was summarized.

<!--truncate-->

## What is Next.js

SSR (server-side rendering framework) based on `React`
...
Similar to `react-router`, including programmatic navigation `router.push` and component-based navigation `<Link href="/about"><a>click me</a></Link>`
...
- **Server-side Rendering** (HTML generated each time a request is made, generated when the user requests it)
...
Considering team collaboration, create a `.vscode` folder in the root directory and create a `setting.json` file inside to achieve the effect of automatic repair, which ensures the unity of the team's code.
