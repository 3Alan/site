...
authors: Alan
summary: >-
  TypeScript 是 JavaScript 的一个超集，支持静态类型检测和面向对象编程。TypeScript
  的基础类型包括布尔值、数字、字符串、数组、元组、枚举、联合类型、类型断言、非空断言、接口、类、泛型等。TypeScript 还支持 React
  相关的内容，如 class 组件动态设置 state 等。
---
