...
draft: true
summary: >-
  - React 新文档的重点是不可变性、组件纯净性、命名风格、hook 原理、setState 命名风格、状态更新队列、通过改变 key
  值来重新渲染、声明式 UI 的 state 设计、避免深度嵌套状态以及 DOM 树更新策略。

  - 可变类型只要当作只读即可，使用 use-immer 来解决嵌套对象书写问题。

  - hook 的简单原理是通过闭包来保存状态。

  - setState 命名风格可以使用首字母缩写或全称。
