...
sidebar_position: 6
summary: >-
  Webpack 性能优化包括减少不必要的 Loader 和 Plugin 的使用、合理使用 Resolve 的 extensions、mainFiles 和
  alias、使用 DllPlugin 优化第三方模块的打包速度、使用多进程打包插件、根据不同环境配置不同的 sourceMap 以及通过
  status.json 文件分析打包过程。
---
