...
keywords:
  - TypeScript 入门教程
  - TypeScript－系统入门到项目实战 笔记总结
description: TypeScript入门教程这一篇就够了，涵盖了 Typescript 的基本用法。
authors: Alan
summary: >-
  TypeScript 是一种 JavaScript 的超集，它在 JavaScript
  的基础上增加了可选的静态类型和基于类的面向对象编程。TypeScript 可以编译成纯 JavaScript，未编译的 ts
  代码无法在浏览器执行。我们把它和 JavaScript 的关系理解成 css 和 less、sass 的关系。


  TypeScript 具有以下优点：


  - 可以进行动态类型检测，可以检测出一些潜在的 bug（例如拼写错误、参数缺失、undefined 等），提升代码健壮性。
  ...
  TypeScript 的基本类型包括：boolean、number、string、数组、元组、枚举、any、void、null 和 undefined 等。


  TypeScript 中的类和 ES6 中的类很相似，不同之处在于 TypeScript
  中的类可以有访问修饰符（public、private、protected）、静态属性和抽象方法。


  TypeScript 中的泛型是指在定义函数、接口或类的时候，不预先指定具体的类型，而在使用的时候再指定类型的一种特性。泛型可以提高代码的可重用性。


  TypeScript 中的 React 相关知识包括：


  - class 组件动态设置 state：可以使用 `this.setState({ [name]: value } as
  Pick<CompontentState, keyof CompontentState>)` 来动态设置 state。
---
