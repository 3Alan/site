...
summary: >-
  本文介绍了如何自定义 Hexo 博客，包括评论功能、后台管理、百度统计、搜索引擎收录及 Vercel 部署。评论功能使用 LeanCloud 国际版和
  Valine-Admin 实现，后台管理使用 hexo-admin，百度统计使用百度统计代码，搜索引擎收录使用
  hexo-submit-urls-to-search-engine，Vercel 部署使用 Vercel 内置的 Hexo 模板。
---
